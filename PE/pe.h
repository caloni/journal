...
public:
   ...
   /// Descobre se um RVA está dentro do range especificado.
   ...
   /// Obtém o endereço do IAT de onde está o endereço de uma determinada função.
   FARPROC* GetIATFunctionAddress(PVOID faddr);

	/// Obtém o endereço da entrada de determinada função através de seu nome.
	FARPROC GetProcAddress(PSTR procName);
	
	/// Obtém o endereço da entrada de determinada função através de seu ordinal.
	FARPROC GetProcAddress(DWORD ordinal);
	
	/// Obtém o endereço da entrada de determinada função através de seu índice.
	FARPROC GetProcAddressByIndex(DWORD index);

	/// Retorna o número de funções exportadas.
	DWORD GetNumberOfFunctions();

	/// Retorna o nome de uma função exportada através de seu endereço.
