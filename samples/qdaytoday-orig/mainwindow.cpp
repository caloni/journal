...
void MainWindow::on_input(const QString &line)
{
    ...
    // no category: the usual daytoday
    if( ! category.empty() )
    {
        ofstream ofs;
        ...
        case kcBlog:
            m_lastPath = ss() << "..\\blog.caloni.github.io\\_posts\\" << GetCurrentDT("%Y-%m-%d") << Slug(title.c_str()) << ".md";
            ofs.open(m_lastPath);
            ofs << "---\n"
                << "title: \"" << title << "\"\n"
                << "---\n";
        ...
        default:
            m_lastPath = ss() << "_posts\\" << GetCurrentDT("%Y-%m-%d") << "-draft-daytoday-" << GetCurrentDT("%H%M%S") << Slug(lines[0]) << ".md";
            ofs.open(m_lastPath);
            ofs << "---\n"
                << "title: \"" << title << "\"\n"
                << "categories: " << category << "\n"
