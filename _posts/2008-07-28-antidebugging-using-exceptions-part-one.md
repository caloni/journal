...
author: admin
comments: true
date: 2008-07-28 09:00:50+00:00
layout: post
slug: antidebugging-using-exceptions-part-one
...
The example bellow receives input through a command prompt and maps the first word typed to the function that must be called. The rest of the typed line is passed as arguments to the functions. The interpreter thread reads the user input and writes into a global string variable, at the same time the executor thread waits the string to be completed to starts the action. It was used the variable pool to let the code simpler, but the ideal would be some kind of synchronise, just like [events](http://msdn.microsoft.com/library/en-us/dllproc/base/createevent.asp), by example. You can download the source code [here](http://www.caloni.com.br/blog/wp-content/uploads/antidebug.cpp).
