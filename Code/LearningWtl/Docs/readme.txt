...
A disposição das pastas visa seguir um modelo padronizado de organização de um projeto (componente standalone) ou componente (que pode ser usado em projetos maiores). É necessário, além de seguir a padronização de pastas, seguir a padronização dos conceitos aqui apresentados.


= Controle de fonte =

Foi criado um commit inicial que visa servir de ponto de partida para a construção do histórico de modificações de projetos. Se todos os projetos começarem do mesmo ponto fica mais fácil fazer futuros processos de refactoring, em que pega-se um projeto e coloca-se em outro e vice-versa. O controle de fonte utilizado é o Bazaar, mas pode ser escolhido qualquer outro distribuído, desde que exista um ponto inicial de commit.
